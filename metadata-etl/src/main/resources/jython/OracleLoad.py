...
class OracleLoad:
  def __init__(self, args):
    ...
    # self.logger.info("Load Oracle Metadata into {}, wh_etl_job_id {}, wh_exec_id {}"
    #                  .format(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id))
    # print "Load Oracle Metadata into %s wh_etl_job_id %s, wh_exec_id %d" %(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id)
  ...
  def delete_types(self):
    ...
    rows = self.conn_cursor.fetchall()
    for row in rows:
      if row[4].encode('utf-8', 'ignore') not in self.csv_urn_list:

        print "%s not in self.csv_urn_list:%s" %(row[4].encode('utf-8', 'ignore'),self.csv_urn_list)
  ...
  def check_dataset_updated(self):
    for row in self.csv_types_list:
      ...
      if len(sqlrows) == 1 and row[1] != sqlrows[0][1].replace("\\","\\\\"):
        ...
        update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,`schema`,`previous_schema`,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}', '{dataset_alias}','{schema}','{previous_schema}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='ORACLE',status='MODIFIED',dataset_id=sqlrow[0],type_name=row[0],index_name=sqlrow[4],dataset_alias=sqlrow[3].replace("'","''") if sqlrow[3] else "",schema=row[1],previous_schema=sqlrow[1].replace("\\","\\\\"),wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
  ...
  def check_dataset_inserted(self):
    ...
    for row in rows:
      ...
      update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}','{dataset_alias}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='ORACLE',status='CREATED',dataset_id=row[0],type_name=row[1],index_name=row[3],dataset_alias='',wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
      print update_log_sql
      self.conn_cursor.execute(update_log_sql.decode('utf-8'))
      self.conn_mysql.commit()
  ...
  def load_tables(self):
   ...
   DELETE FROM stg_dict_dataset WHERE wh_etl_job_id = {wh_etl_job_id};
    ...
    print "finish loading Oracle type metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def load_fields(self):
    ...
    print "finish loading Oracle field metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def run(self):
    try:
      ...
      print "Load Oracle metadata [%s -> %s]" % (str(begin), str(end))
    finally:
