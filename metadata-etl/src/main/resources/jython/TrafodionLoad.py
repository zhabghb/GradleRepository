...
class TrafodionLoad:
  def __init__(self, args):
    ...
    print JDBC_URL
    self.input_table_file = args[Constant.TRAFODION_SCHEMA_OUTPUT_KEY]
    print self.input_table_file
    self.input_field_file = args[Constant.TRAFODION_FIELD_OUTPUT_KEY]
    ...
    # self.logger.info("Load TRAFODION Metadata into {}, wh_etl_job_id {}, wh_exec_id {}"
    #                  .format(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id))
    # print "Load TRAFODION Metadata into %s wh_etl_job_id %s, wh_exec_id %d" %(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id)
  ...
  def check_dataset_updated(self):
    for row in self.csv_types_list:
      ...
      if len(sqlrows) == 1 and row[1].replace("\"","") != sqlrows[0][1].replace("\"",""):
        ...
        update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,`schema`,`previous_schema`,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}', '{dataset_alias}','{schema}','{previous_schema}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='TRAFODION',status='MODIFIED',dataset_id=sqlrow[0],type_name=row[0],index_name=sqlrow[4],dataset_alias=sqlrow[3].replace("'","''") if sqlrow[3] else "",schema=json.dumps(format_json),previous_schema=sqlrow[1],wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
  ...
  def check_dataset_inserted(self):
    ...
    for row in rows:
      ...
      update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}','{dataset_alias}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='TRAFODION',status='CREATED',dataset_id=row[0],type_name=row[1],index_name=row[3],dataset_alias='',wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
      print update_log_sql
  ...
  def load_tables(self):
   ...
   DELETE FROM stg_dict_dataset WHERE wh_etl_job_id = {wh_etl_job_id};
    ...
    # self.logger.info("finish loading TRAFODION table metadata from {} to {}"
    #                  .format(self.input_table_file, self.dict_dataset_table))
    print "finish loading TRAFODION type metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def load_fields(self):
    ...
    # self.logger.info("finish loading TRAFODION type fields from {} to {}"
    #                  .format(self.input_field_file, self.dict_field_table))
    print "finish loading TRAFODION field metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def run(self):
    try:
      ...
      # self.logger.info("Load TRAFODION metadata [%s -> %s]" % (str(begin), str(end)))
      print "Load TRAFODION metadata [%s -> %s]" % (str(begin), str(end))
    finally:
