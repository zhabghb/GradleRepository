...
class HiveLoad:
  def __init__(self, args):
    ...
    print JDBC_URL
    self.input_table_file = args[Constant.HIVE_SCHEMA_CSV_FILE_KEY]
    print self.input_table_file
    self.input_field_file = args[Constant.HIVE_FIELD_METADATA_KEY]
    ...
    # self.logger.info("Load Hive Metadata into {}, wh_etl_job_id {}, wh_exec_id {}"
    #                  .format(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id))
    # print "Load Hive Metadata into %s wh_etl_job_id %s, wh_exec_id %d" %(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id)
  ...
  def check_dataset_updated(self):
    for row in self.csv_types_list:
      ...
      if len(sqlrows) == 1 and row[1] != sqlrows[0][1]:
        sqlrow = sqlrows[0]
        print "======there is an update======="
        print row[1]
        print sqlrow[1]

        update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,`schema`,`previous_schema`,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}', '{dataset_alias}','{schema}','{previous_schema}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='HIVE',status='MODIFIED',dataset_id=sqlrow[0],type_name=row[0],index_name=sqlrow[4],dataset_alias=sqlrow[3].replace("'","''") if sqlrow[3] else "",schema=row[1],previous_schema=sqlrow[1],wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
  ...
  def check_dataset_inserted(self):
    self.conn_cursor.execute("SELECT id,name,alias,parent_name from dict_dataset where modified_time is null and (parent_name is not null and char_length(trim(parent_name)) <> 0 and parent_name <> 'null') and wh_etl_job_id = "+str(self.wh_etl_job_id))
    rows = self.conn_cursor.fetchall()
    for row in rows:
      print "======there is an insert======="
      update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}','{dataset_alias}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),dataset_type='HIVE',status='CREATED',dataset_id=row[0],type_name=row[1],index_name=row[3],dataset_alias='',wh_etl_job_id=self.wh_etl_job_id,wh_etl_exec_id=self.wh_etl_exec_id,username=self.sync_username)
      print update_log_sql
  ...
  def load_tables(self):
   ...
   DELETE FROM stg_dict_dataset WHERE wh_etl_job_id = {wh_etl_job_id};
    ...
    `storage_type`, `dataset_type`, `is_partitioned`,`source_created_time`,`source_modified_time`)
    ...
    # self.logger.info("finish loading Hive table metadata from {} to {}"
    #                  .format(self.input_table_file, self.dict_dataset_table))
    print "finish loading Hive type metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def load_fields(self):
    load_fields_cmd = '''
        ...
        FIELDS TERMINATED BY '\Z'
        (urn, sort_id, parent_sort_id,parent_path,field_name, data_type, is_nullable, @vdata_size,is_indexed,is_partitioned,
        @vdefault_value, @remark)
        SET wh_etl_job_id = {wh_etl_job_id},
    ...
    # self.logger.info("finish loading Hive type fields from {} to {}"
    #                  .format(self.input_field_file, self.dict_field_table))
    print "finish loading Hive field metadata from %s to %s" %(self.input_table_file, self.dict_dataset_table)
  ...
  def run(self):
    try:
      ...
      # self.logger.info("Load Hive metadata [%s -> %s]" % (str(begin), str(end)))
      print "Load Hive metadata [%s -> %s]" % (str(begin), str(end))
    finally:
...
if __name__ == "__main__":
  try:
    ...
    print "===========[warn]===========During hive load process ,catch IndexError ==> sys.argv[1] "
    print len(sys.argv)
    print args
