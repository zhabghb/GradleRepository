...
class TrafodionLoad:
    def __init__(self, args):
        ...
        print JDBC_URL
        self.input_table_file = args[Constant.PGXZ_SCHEMA_OUTPUT_KEY]
        print self.input_table_file
        self.input_field_file = args[Constant.PGXZ_FIELD_OUTPUT_KEY]
        print self.input_field_file
        self.csv_urn_list = []
        self.csv_types_list = []
        # self.input_table_file = "/var/tmp/wherehows/type_metadata.dat"
        # self.input_field_file = "/var/tmp/wherehows/field_metadata.dat"
        ...
        # self.logger.info("Load PGXZ Metadata into {}, wh_etl_job_id {}, wh_exec_id {}"
        #                  .format(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id))
        # print "Load PGXZ Metadata into %s wh_etl_job_id %s, wh_exec_id %d" %(JDBC_URL, self.wh_etl_job_id, self.wh_etl_exec_id)
    ...
    def load_csv(self):
        ...
        for index, line in enumerate(reader):
            if index > 0:
                ...
                self.csv_types_list.append(rows)

        csvfile.close()

    def insert_dataset_tag(self):
        self.conn_cursor.execute(
            "SELECT id from dict_dataset where modified_time is null and (parent_name is not null and char_length(trim(parent_name)) <> 0) and wh_etl_job_id = " + str(
                self.wh_etl_job_id))
        rows = self.conn_cursor.fetchall()
        for row in rows:
            dataset_tag_sql = "INSERT INTO `dataset_tag`(dataset_id,tag_id) VALUES ('{dataset_id}', '{tag_id}')".format(
                dataset_id=row[0], tag_id=self.wh_etl_job_id)
    ...
    def delete_types(self):
        self.conn_cursor.execute(
            "SELECT id,name,alias,parent_name,urn from dict_dataset where wh_etl_job_id = " + str(self.wh_etl_job_id))
        rows = self.conn_cursor.fetchall()
        for row in rows:
            if row[4] not in self.csv_urn_list:
                ...
                if row[3] and row[3] != "":
                    sql4 = "INSERT INTO `stage_log`(update_time,type,status,dataset_name,parent_name,dataset_alias,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}', '{type_name}', '{index_name}', '{dataset_alias}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(
                        update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), dataset_type='PGXZ',
                        status='DELETED', type_name=row[1], index_name=row[3],
                        dataset_alias=row[2].replace("'", "''") if row[2] else "", wh_etl_job_id=self.wh_etl_job_id,
                        wh_etl_exec_id=self.wh_etl_exec_id, username=self.sync_username)
    ...
    def check_dataset_updated(self):
        for row in self.csv_types_list:
            sql = "SELECT d.id,d.schema,d.name,d.alias,d.parent_name from dict_dataset d where urn='{urn}'".format(
                urn=row[4])
            self.conn_cursor.execute(sql)
            sqlrows = self.conn_cursor.fetchall()
            if len(sqlrows) == 1 and row[1].replace("\"", "") != sqlrows[0][1].replace("\"", ""):
                sqlrow = sqlrows[0]
                ...
                update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,`schema`,`previous_schema`,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}', '{dataset_alias}','{schema}','{previous_schema}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(
                    update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), dataset_type='PGXZ',
                    status='MODIFIED', dataset_id=sqlrow[0], type_name=row[0], index_name=sqlrow[4],
                    dataset_alias=sqlrow[3].replace("'", "''") if sqlrow[3] else "", schema=json.dumps(format_json,ensure_ascii=False),
                    previous_schema=sqlrow[1], wh_etl_job_id=self.wh_etl_job_id, wh_etl_exec_id=self.wh_etl_exec_id,
                    username=self.sync_username)
                print update_log_sql
                self.conn_cursor.execute(update_log_sql.decode('utf-8'))
                self.conn_mysql.commit()

                update_dataset_sql = "UPDATE dict_dataset AS a SET previous_schema = a.`schema` WHERE id ={id} ".format(
                    id=sqlrow[0])
    ...
    def check_dataset_inserted(self):
        self.conn_cursor.execute(
            "SELECT id,name,alias,parent_name from dict_dataset where modified_time is null and (parent_name is not null and char_length(trim(parent_name)) <> 0) and wh_etl_job_id = " + str(
                self.wh_etl_job_id))
        rows = self.conn_cursor.fetchall()
        for row in rows:
            print "======there is an insert======="
            update_log_sql = "INSERT INTO `stage_log`(update_time,type,status,dataset_id,dataset_name,parent_name,dataset_alias,wh_etl_job_id,wh_etl_exec_id,username) VALUES ('{update_time}', '{dataset_type}','{status}','{dataset_id}', '{type_name}', '{index_name}','{dataset_alias}', '{wh_etl_job_id}', '{wh_etl_exec_id}','{username}')".format(
                update_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), dataset_type='PGXZ',
                status='CREATED', dataset_id=row[0], type_name=row[1], index_name=row[3], dataset_alias='',
                wh_etl_job_id=self.wh_etl_job_id, wh_etl_exec_id=self.wh_etl_exec_id, username=self.sync_username)
            print update_log_sql
            self.conn_cursor.execute(update_log_sql.decode('utf-8'))
            self.conn_mysql.commit()
   ...
   DELETE FROM stg_dict_dataset WHERE wh_etl_job_id = {wh_etl_job_id};
    ...
    '''.format(wh_etl_job_id=self.wh_etl_job_id, source_file=self.input_table_file, wh_etl_exec_id=self.wh_etl_exec_id,
        ...
        # self.logger.info("finish loading PGXZ table metadata from {} to {}"
        #                  .format(self.input_table_file, self.dict_dataset_table))
        print "finish loading PGXZ type metadata from %s to %s" % (self.input_table_file, self.dict_dataset_table)
    ...
    def load_fields(self):
        ...
        where wh_etl_job_id = {wh_etl_job_id}
        ...
        insert into {dict_field_detail} (
          ...
          field_name, data_type, data_size, is_nullable, default_value, modified,wh_etl_job_id
        )
        select
          d.id, sf.sort_id, sf.parent_sort_id, sf.parent_path,
          sf.field_name, sf.data_type, sf.data_size, sf.is_nullable, sf.default_value, now(),sf.wh_etl_job_id
    ...
    '''.format(source_file=self.input_field_file, wh_etl_job_id=self.wh_etl_job_id,
        ...
        # self.logger.info("finish loading PGXZ type fields from {} to {}"
        #                  .format(self.input_field_file, self.dict_field_table))
        print "finish loading PGXZ field metadata from %s to %s" % (self.input_table_file, self.dict_dataset_table)
    ...
    def run(self):
        try:
            ...
            sql = "SELECT wh_etl_exec_id FROM wh_etl_job_execution WHERE wh_etl_job_id = {wh_etl_job_id} and `status` = 'SUCCEEDED'".format(
                wh_etl_job_id=self.wh_etl_job_id)
            self.conn_cursor.execute(sql)
            rows = self.conn_cursor.fetchall()
            if len(rows) > 0:
            ...
            # self.logger.info("Load PGXZ metadata [%s -> %s]" % (str(begin), str(end)))
            print "Load PGXZ metadata [%s -> %s]" % (str(begin), str(end))
        finally:
